...
+ (UIColor *)hexColorWithHex:(NSString *)hex alpha:(CGFloat)alpha {
  ...
  if (dict) {
    return [[self alloc] initWithRed:[dict[@"red"] floatValue] green:[dict[@"green"] floatValue] blue:[dict[@"blue"] floatValue] alpha:alpha];
  } else {
    return nil;
  }
}
...
+ (UIColor *)hexColorWithHex:(NSString *)hex {
  ...
  NSDictionary<NSString *, NSNumber *> *dict = [self transformWithHex:hex];
  if (dict) {
    return [[self alloc] initWithRed:[dict[@"red"] floatValue] green:[dict[@"green"] floatValue] blue:[dict[@"blue"] floatValue] alpha:[dict[@"alpha"] floatValue]];
  } else {
    return nil;
  }
}
