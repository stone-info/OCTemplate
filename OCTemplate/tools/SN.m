...
+ (NSString *(^)(NSString *))urlDecoding {
  return ^NSString *(NSString *str) {
    return (__bridge_transfer NSString *) CFURLCreateStringByReplacingPercentEscapesUsingEncoding(NULL, (__bridge CFStringRef) str, CFSTR(""), CFStringConvertNSStringEncodingToEncoding(NSUTF8StringEncoding));
  };
}
 ...
 * @param completed
 * @example
 * [sn promiseAllTest:^(dispatch_group_t group, dispatch_queue_t queue) {

    dispatch_group_async(group, queue, ^{
      dispatch_group_enter(group);

      [YJRequestManager shareFollowerListWithParams:nil completed:^(NSString *jsonString, NSError *error) {
...
+ (void)promiseAll:(void (^)(dispatch_group_t group, dispatch_queue_t queue))taskList completed:(void (^)(void))completed {
  ...
  dispatch_queue_t queue = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
  //3.添加请求
  // [self foo:group queue:queue];

  !taskList ?: taskList(group, queue);
...
+ (void)promiseTestAll:(void (^)(dispatch_group_t group, dispatch_queue_t queue, NSMutableArray *errorGroup))taskList completed:(void (^)(NSMutableArray *errorGroup))completed {
  NSMutableArray   *errorGroup = [NSMutableArray array];
  //1.创建队列组
  dispatch_group_t group       = dispatch_group_create();
  //2.创建队列
  dispatch_queue_t queue       = dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
  ...
  !taskList ?: taskList(group, queue, errorGroup);

  //4.队列组所有请求完成回调刷新UI
  dispatch_group_notify(group, dispatch_get_main_queue(), ^{
    !completed ?: completed(errorGroup);
  });
