...
#import "T087ViewController.h"
#import "SNJSOCModel.h"

@interface T087ViewController ()<UIWebViewDelegate, UIScrollViewDelegate>
@property (nonatomic, weak) UIWebView *  webView;
@property (nonatomic, strong) JSContext *jsContext;
@end

@implementation T087ViewController
- (void)catchJsLog {
  if (DEBUG) {
    ...
    ctx[@"h5GroupBegin"] = ^(JSValue *obj, JSValue *key, JSValue *filename, JSValue *lineNumber, JSValue *color) {
      ...
      NSString *emoji;
      if ([key.toString containsString:@"req"]) {
        emoji = @"🦋";
      }
      if ([key.toString containsString:@"res"]) {
        emoji = @"🐬";
      }
      if ([key.toString containsString:@"error"]) {
        emoji = @"💊";
      }
...
- (void)viewDidLoad {
  ...
  // NSURL *url = [NSURL URLWithString:@"http://192.168.1.105:7000"];
  NSURL *url = [NSURL URLWithString:@"http://192.168.1.105:8080"];
