...
- (NSData *)cc_encryptUsingAlgorithm:(CcCryptoAlgorithm)algorithm key:(id)key InitializationVector:(id)iv Mode:(CcCryptorMode)mode Padding:(CcCryptorPadding)padding {

  CCCryptorStatus status = kCCSuccess;

  NSData *result = [self cc_cryptologyUsingOperation:kCCEncrypt Algorithm:algorithm Padding:padding Mode:mode key:key initializationVector:iv error:&status];
...
- (NSData *)cc_decryptUsingAlgorithm:(CcCryptoAlgorithm)algorithm key:(id)key InitializationVector:(id)iv Mode:(CcCryptorMode)mode Padding:(CcCryptorPadding)padding {
...
- (NSData *)cc_cryptologyUsingOperation:(CCOperation)operation Algorithm:(CCAlgorithm)algorithm Padding:(CcCryptorPadding)padding Mode:(CcCryptorMode)mode key:(id)key initializationVector:(id)iv error:(CCCryptorStatus *)error {
  ...
  status = CCCryptorUpdate(cryptor, [sourceData bytes], (size_t) [sourceData length], buf, bufsize, &bufused);
